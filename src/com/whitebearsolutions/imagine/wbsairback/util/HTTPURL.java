 ...
 */ 
    public static String encode(String s) {
    	if(s == null) { return ""; }
    	StringBuilder sencode = new StringBuilder();
        for(int i = 0; i < s.length(); i++) {
        	int c = s.charAt(i);
            if(c >= 48 && c <= 57) {
            	sencode.append((char) c);
            } else if(c >= 65 && c <= 90) {
            	sencode.append((char) c);
            } else if(c >= 97 && c <= 122) {
            	sencode.append((char) c);
            } else if(c == 32) {
            	sencode.append(hex[c]);
            } else if(c <= 127) {
            	sencode.append(hex[c]);
            } else if(c <= 2047) {
            	sencode.append(hex[0xc0 | c >> 6]);
                sencode.append(hex[0x80 | c & 0x3f]);
            } else {
            	sencode.append(hex[0xe0 | c >> 12]);
            	sencode.append(hex[0x80 | c >> 6 & 0x3f]);
            	sencode.append(hex[0x80 | c & 0x3f]);
            }
        }
        return sencode.toString();
    }
