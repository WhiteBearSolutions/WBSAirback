...
ImagineWBSPassword::ImagineWBSPassword(ImagineWBSWindow *window) {
...
ImagineWBSPassword::~ImagineWBSPassword(void) {
	int nfields = ARRAY_SIZE(fields) - 1;
	free_form(form);
	for(int i = 0; i < nfields; i++) {
		free_field(fields[i]);
...
void ImagineWBSPassword::createWindow(void) {
	...
	mvprintw(LINES - 3, 0, "                                      ");
	mvprintw(LINES - 3, 0, "Press <ENTER> to save password");
	refresh();
}

void ImagineWBSPassword::writePassword(void) {
	string password = systemTools->trim(field_buffer(fields[0],0));
	systemTools->changeRootPassword((char*)password.c_str());
}

void ImagineWBSPassword::destroyWindow(void) {
	endwin();
	werase(form_win);
	wrefresh(form_win);
	repaintParentWindow();
...
void ImagineWBSPassword::displayContent(void) {
	set_form_win(form, form_win);
	set_form_sub(form, derwin(form_win, 3, 30, 2, 2));
	post_form(form);

	mvwprintw(form_win, 2, 5, "New password:");
	wrefresh(form_win);
	refresh();
}

void ImagineWBSPassword::hideContent(void) {
	unpost_form(form);
}

void ImagineWBSPassword::show(void) {
	createWindow();
	displayContent();
	...
	while(display) {
		...
		switch(c) {
			...
			case 10: {
				...
				_m->setMessage((char*)"Root password saved");
				_m->show();
				free(_m);
				display = false;
				}
