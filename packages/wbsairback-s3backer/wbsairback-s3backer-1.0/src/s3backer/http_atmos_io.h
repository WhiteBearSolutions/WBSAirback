...
struct http_atmos_io_stats {
    ...
    /* HTTP transfer stats */
    struct http_atmos_io_evst http_heads;                 // total successful
    struct http_atmos_io_evst http_gets;                  // total successful
    struct http_atmos_io_evst http_puts;                  // total successful
    struct http_atmos_io_evst http_deletes;               // total successful
...
/* http_io.c */
extern struct s3backer_store *http_atmos_io_create(struct http_atmos_io_conf *config);
extern void http_atmos_io_get_stats(struct s3backer_store *s3b, struct http_atmos_io_stats *stats);
extern int http_atmos_io_detect_sizes(struct s3backer_store *s3b, off_t *file_sizep, u_int *block_sizep);
extern int http_atmos_io_parse_block(struct http_atmos_io_conf *config, const char *name, s3b_block_t *block_num);
extern void getHash(char* hashString, struct curl_slist *headers, const char* method,const char* datebuf, const char* url, const char* contentType);
