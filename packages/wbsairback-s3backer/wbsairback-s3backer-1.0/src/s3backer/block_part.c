 ...
 * 02110-1301, USA.
 *
 * $Id: block_part.c 448 2011-02-01 17:31:15Z archie.cobbs $
 */
    ...
    u_int block_size, u_int off, u_int len, const void *src)
{
    ...
    if (len == 0)
        return 0;
    if (off == 0 && len == block_size)
        return (*s3b->write_block)(s3b, block_num, src, NULL, NULL, NULL, 0);
    ...
    if ((r = (*s3b->write_block)(s3b, block_num, buf, NULL, NULL, NULL, 0)) != 0) {
        free(buf);
        return r;
    }
