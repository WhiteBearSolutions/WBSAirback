...
/* ATMOS URL */
#define ATMOS_BASE_URL                              "https://api.atmosonline.com/rest/namespace/"
#define ATMOS_DEFAULT_PREFIX                     	"root"
#define ATMOS_DEFAULT_PWD_FILE                      ".s3backer_atmos_passwd"
...
static struct s3b_config config = {
	...
	.http_atmos_io= {
		.accessId=              NULL,
		.accessKey=             NULL,
		.baseURL=               ATMOS_BASE_URL,
		.prefix=                ATMOS_DEFAULT_PREFIX,
...
static const struct fuse_opt option_list[] = {
    ...
    {
        .templ=     "--prefixAtmos=%s",
        .offset=    offsetof(struct s3b_config, http_atmos_io.prefix),
    },
    {
        .templ=     "--prefix=%s",
    ...
    {
        .templ=     "--atmos",
        .offset=    offsetof(struct s3b_config, atmos),
        .value=     1
    },
    {
        .templ=     "--timeout=%u",
...
struct s3backer_store *http_atmos_io_store;
...
s3backer_create_store(struct s3b_config *conf)
{
    ...
    if (conf->test) {
        ...
        if ((http_atmos_io_store = http_atmos_io_create(&conf->http_atmos_io)) == NULL)
            return NULL;
        store = http_atmos_io_store;
    }
...
s3b_config_print_stats(void *prarg, printer_t *printer)
{
    ...
    struct http_atmos_io_stats http_atmos_io_stats;
    ...
    if (http_atmos_io_store != NULL)
    	http_atmos_io_get_stats(http_atmos_io_store, &http_atmos_io_stats);
    ...
    if (http_atmos_io_store != NULL) {
    	(*printer)(prarg, "%-28s \n", "ATMOS HTTP STATS");
        (*printer)(prarg, "%-28s %u\n", "http_normal_blocks_read", http_atmos_io_stats.normal_blocks_read);
        (*printer)(prarg, "%-28s %u\n", "http_normal_blocks_written", http_atmos_io_stats.normal_blocks_written);
        (*printer)(prarg, "%-28s %u\n", "http_zero_blocks_read", http_atmos_io_stats.zero_blocks_read);
        (*printer)(prarg, "%-28s %u\n", "http_zero_blocks_written", http_atmos_io_stats.zero_blocks_written);
        if (config.list_blocks) {
            (*printer)(prarg, "%-28s %u\n", "http_empty_blocks_read", http_atmos_io_stats.empty_blocks_read);
            (*printer)(prarg, "%-28s %u\n", "http_empty_blocks_written", http_atmos_io_stats.empty_blocks_written);
        }
        (*printer)(prarg, "%-28s %u\n", "http_gets", http_atmos_io_stats.http_gets.count);
        (*printer)(prarg, "%-28s %u\n", "http_puts", http_atmos_io_stats.http_puts.count);
        (*printer)(prarg, "%-28s %u\n", "http_deletes", http_atmos_io_stats.http_deletes.count);
        (*printer)(prarg, "%-28s %.3f sec\n", "http_avg_get_time", http_atmos_io_stats.http_gets.count > 0 ?
        		http_atmos_io_stats.http_gets.time / http_atmos_io_stats.http_gets.count : 0.0);
        (*printer)(prarg, "%-28s %.3f sec\n", "http_avg_put_time", http_atmos_io_stats.http_puts.count > 0 ?
        		http_atmos_io_stats.http_puts.time / http_atmos_io_stats.http_puts.count : 0.0);
        (*printer)(prarg, "%-28s %.3f sec\n", "http_avg_delete_time", http_atmos_io_stats.http_deletes.count > 0 ?
        		http_atmos_io_stats.http_deletes.time / http_atmos_io_stats.http_deletes.count : 0.0);
        (*printer)(prarg, "%-28s %u\n", "http_unauthorized", http_atmos_io_stats.http_unauthorized);
        (*printer)(prarg, "%-28s %u\n", "http_forbidden", http_atmos_io_stats.http_forbidden);
        (*printer)(prarg, "%-28s %u\n", "http_stale", http_atmos_io_stats.http_stale);
        (*printer)(prarg, "%-28s %u\n", "http_verified", http_atmos_io_stats.http_verified);
        (*printer)(prarg, "%-28s %u\n", "http_mismatch", http_atmos_io_stats.http_mismatch);
        (*printer)(prarg, "%-28s %u\n", "http_5xx_error", http_atmos_io_stats.http_5xx_error);
        (*printer)(prarg, "%-28s %u\n", "http_4xx_error", http_atmos_io_stats.http_4xx_error);
        (*printer)(prarg, "%-28s %u\n", "http_other_error", http_atmos_io_stats.http_other_error);
        (*printer)(prarg, "%-28s %u\n", "http_canceled_writes", http_atmos_io_stats.http_canceled_writes);
        (*printer)(prarg, "%-28s %u\n", "http_num_retries", http_atmos_io_stats.num_retries);
        (*printer)(prarg, "%-28s %ju.%03u sec\n", "http_total_retry_delay",
          (uintmax_t)(http_atmos_io_stats.retry_delay / 1000), (u_int)(http_atmos_io_stats.retry_delay % 1000));
        total_curls = http_atmos_io_stats.curl_handles_created + http_atmos_io_stats.curl_handles_reused;
        if (total_curls > 0)
            curl_reuse_ratio = (double)http_atmos_io_stats.curl_handles_reused / (double)total_curls;
        (*printer)(prarg, "%-28s %.4f\n", "curl_handle_reuse_ratio", curl_reuse_ratio);
        (*printer)(prarg, "%-28s %u\n", "curl_timeouts", http_atmos_io_stats.curl_timeouts);
        (*printer)(prarg, "%-28s %u\n", "curl_connect_failed", http_atmos_io_stats.curl_connect_failed);
        (*printer)(prarg, "%-28s %u\n", "curl_host_unknown", http_atmos_io_stats.curl_host_unknown);
        (*printer)(prarg, "%-28s %u\n", "curl_out_of_memory", http_atmos_io_stats.curl_out_of_memory);
        (*printer)(prarg, "%-28s %u\n", "curl_other_error", http_atmos_io_stats.curl_other_error);
        total_oom += http_atmos_io_stats.out_of_memory_errors;
    }
...
validate_config(void)
{
    ...
    if (config.accessFile == NULL) {
        ...
        if (home != NULL) {
            if (config.atmos) {
            	snprintf(buf, sizeof(buf), "%s/%s", home, ATMOS_DEFAULT_PWD_FILE);
    ...
    if (config.atmos) {
    	config.http_atmos_io.baseURL = config.http_io.baseURL;
    	/* If no accessId specified, default to first in accessFile */
		if (config.http_atmos_io.accessId == NULL && config.accessFile != NULL)
			search_access_for(config.accessFile, NULL, &config.http_atmos_io.accessId, NULL);
		if (config.http_atmos_io.accessId != NULL && *config.http_atmos_io.accessId == '\0')
			config.http_atmos_io.accessId = NULL;
		if (config.http_atmos_io.accessId == NULL
		  && !config.fuse_ops.read_only
		  && (strcmp(config.http_atmos_io.baseURL, ATMOS_BASE_URL) == 0 )) {
		...
		if (config.http_atmos_io.accessId == NULL && config.http_atmos_io.accessKey != NULL) {
			warnx("an `accessKey' was specified but no `accessId' was specified");
			return -1;
		}
		if (config.http_atmos_io.accessId != NULL) {
			if (config.http_atmos_io.accessKey == NULL && config.accessFile != NULL)
				search_access_for(config.accessFile, config.http_atmos_io.accessId, NULL, &config.http_atmos_io.accessKey);
			if (config.http_atmos_io.accessKey == NULL) {
    ...
    if (!config.atmos) {
    	...
    	s = NULL;
		if (strncmp(config.http_atmos_io.baseURL, "http://", 7) == 0)
			s = config.http_atmos_io.baseURL + 7;
		else if (strncmp(config.http_atmos_io.baseURL, "https://", 8) == 0)
			s = config.http_atmos_io.baseURL + 8;
		if (s != NULL && (*s == '/' || *s == '\0'))
			s = NULL;
		if (s != NULL && (s = strrchr(s, '/')) == NULL) {
			warnx("base URL must end with a '/'");
		...
		if (s == NULL) {
			warnx("invalid base URL `%s'", config.http_atmos_io.baseURL);
			return -1;
		}
		if (config.ssl) {
			if (strcmp(config.http_atmos_io.baseURL, ATMOS_BASE_URL) != 0) {
				warnx("specify only one of `--baseURL' and `--ssl'");
				return -1;
			}
			config.http_atmos_io.baseURL = ATMOS_BASE_URL;
		}
    }

    /* Handle virtual host style URL (prefix hostname with bucket name) */
    if (!config.atmos && config.http_io.vhost) {
        size_t buflen;
        int schemelen;
        char *buf;
    ...
    if (config.test) {
        ...
        snprintf(config.description, sizeof(config.description), "%s%s/%s",
    	 ...
    	 snprintf(config.description, sizeof(config.description), "%s%s",
    	          config.http_atmos_io.baseURL, config.http_atmos_io.prefix);
    }
    ...
    if (config.no_auto_detect)
    	...
    	config.http_atmos_io.debug = config.debug;
		config.http_atmos_io.quiet = config.quiet;
		config.http_atmos_io.log = config.log;
		if ((s3b = http_atmos_io_create(&config.http_atmos_io)) == NULL)
			err(1, "http_atmos_io_create");
		if (!config.quiet)
			warnx("auto-detecting block size and total file size...");
		r = http_atmos_io_detect_sizes(s3b, &auto_file_size, &auto_block_size);
		(*s3b->destroy)(s3b);
    }
    ...
    if (config.http_atmos_io.encryption != NULL && config.block_size % EVP_MAX_IV_LENGTH != 0) {
        warnx("block size must be at least %u when encryption is enabled", EVP_MAX_IV_LENGTH);
        return -1;
    }
    ...
    if (!config.atmos) {
    	...
    	config.http_atmos_io.debug = config.debug;
		config.http_atmos_io.quiet = config.quiet;
		config.http_atmos_io.block_size = config.block_size;
		config.http_atmos_io.num_blocks = config.num_blocks;
		config.http_atmos_io.log = config.log;
    }
...
dump_config(void)
{
    ...
    if (!config.atmos) {
    	...
    	(*config.log)(LOG_DEBUG, "%24s: \"%s\"", "accessId", config.http_atmos_io.accessId != NULL ? config.http_io.accessId : "");
    	(*config.log)(LOG_DEBUG, "%24s: \"%s\"", "accessKey", config.http_atmos_io.accessKey != NULL ? "****" : "");
    }
    ...
    if (!config.atmos) {
    	...
    	(*config.log)(LOG_DEBUG, "%24s: %s", "accessType", config.http_atmos_io.accessType);
    	(*config.log)(LOG_DEBUG, "%24s: \"%s\"", "baseURL", config.http_atmos_io.baseURL);
    }
    ...
    if (!config.atmos)
    	...
    	(*config.log)(LOG_DEBUG, "%24s: \"%s\"", "prefix", config.http_atmos_io.prefix);
    ...
    if (!config.atmos) {
    	...
    	(*config.log)(LOG_DEBUG, "%24s: %d", "compress", config.http_atmos_io.compress);
		(*config.log)(LOG_DEBUG, "%24s: %s", "encryption", config.http_atmos_io.encryption != NULL ? config.http_io.encryption : "(none)");
		(*config.log)(LOG_DEBUG, "%24s: \"%s\"", "password", config.http_atmos_io.password != NULL ? "****" : "");
    }
    ...
    if (!config.atmos) {
		...
		(*config.log)(LOG_DEBUG, "%24s: %us", "timeout", config.http_atmos_io.timeout);
		(*config.log)(LOG_DEBUG, "%24s: %ums", "initial_retry_pause", config.http_atmos_io.initial_retry_pause);
		(*config.log)(LOG_DEBUG, "%24s: %ums", "max_retry_pause", config.http_atmos_io.max_retry_pause);
    }
...
usage(void)
{
    ...
    fprintf(stderr, "\ts3backer [options] --atmos /mount/point\n");
