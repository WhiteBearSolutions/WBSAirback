 ...
 * 02110-1301, USA.
 *
 * $Id: tester.c 448 2011-02-01 17:31:15Z archie.cobbs $
 */
...
main(int argc, char **argv)
{
    ...
    for (i = 0; i < config->num_blocks; i++) {
        ...
        if ((r = (*store->write_block)(store, i, zero_block, NULL, NULL, NULL, 0)) != 0)
            err(1, "write error");
    }
    ...
    while (1) {
        ...
        if ((random() % READ_FACTOR) != 0) {
            ...
            logit(id, "wr %0*jx content=0x%02x%02x%02x%02x START\n", S3B_BLOCK_NUM_DIGITS, (uintmax_t)block_num,
              data[0], data[1], data[2], data[3]);
            if ((r = (*store->write_block)(store, block_num, data, NULL, NULL, NULL, 0)) != 0)
