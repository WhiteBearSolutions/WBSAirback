...
s3backer_erase(struct s3b_config *config)
{
    ...
    if (config->atmos) {
    	if ((priv->s3b = http_atmos_io_create(&config->http_atmos_io)) == NULL) {
    		 goto fail3;
    	}
    } else if ((priv->s3b = config->test ? test_io_create(&config->http_io) : http_io_create(&config->http_io)) == NULL) {
...
erase_thread_main(void *arg)
{
    ...
    while (1) {
        ...
        if (priv->qlen > 0) {
            ...
            //fprintf(stderr,"En teoría, borramos %d ... \n",block_num);
            /* Do block deletion */
            pthread_mutex_unlock(&priv->mutex);
            r = (*priv->s3b->write_block)(priv->s3b, block_num, NULL, NULL, NULL, NULL, 0);
            pthread_mutex_lock(&priv->mutex);
