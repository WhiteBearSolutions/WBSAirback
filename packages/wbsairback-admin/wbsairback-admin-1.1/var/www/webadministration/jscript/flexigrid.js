	...
	$.addFlex = function (t, p) {
		...
		var g = {
			...
			populate: function () { //get latest data
				...
				if($.cookie(p.tableId+'_page')) p.newp = $.cookie(p.tableId+'_page');
				if($.cookie(p.tableId+'_rp')) p.rp = $.cookie(p.tableId+'_rp');
				if (!p.newp) {
					p.newp = 1;
				}
				if (p.page > p.pages) {
			...
			changePage: function (ctype) { //change page
				...
				if (p.onChangePage) {
					...
					$.cookie(p.tableId+'_page', p.newp, { expires: date });
					this.populate();
				}
			},
		...
		if (p.usepager) {
			...
			if (p.useRp) {
				...
				$('select', g.pDiv).change(function () {
					if (p.onRpChange) {
						...
						p.newp = 1;
						p.rp = +this.value;
						$.cookie(p.tableId+'_rp', p.rp, { expires: date });
						g.populate();
					}
				});
			}
			...
			if (p.searchitems) {
				...
				$('input[name=q]', g.sDiv).keydown(function (e) {
					if (e.keyCode == 13) {
						if($.cookie(p.tableId+'_page')) $.cookie(p.tableId+'_page', null);
						if($.cookie(p.tableId+'_rp')) $.cookie(p.tableId+'_rp', null);
						g.doSearch();
					}
				});
